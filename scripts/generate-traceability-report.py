...
MODEL_RELIABILITY_LIMITATIONS = [
    "Accepted annotations and prior human-review artifacts remain distinct from multi-model diagnostic outputs.",
    "Model agreement does not establish human inter-annotator reliability, scholarly reproducibility, or evidentiary correctness.",
...
def write_model_reliability_disclosure(generated_at: str) -> None:
    text = f"""---
...
model-reliability disclosure, data-availability statement, validation gate,
and public-readiness audit for scholarly review. It preserves draft boundaries
rather than promoting the Lincoln pilot analysis as final.
...
def main() -> int:
    ...
    write_model_reliability_disclosure(generated_at)
